...
if [ $? -eq 1 ]; then
	...
	if [ -z "$(echo $(($phycore*2)) | grep $nproc)" ]; then
		write_log "Does not look like you have HTi/AMT Enabled"
		if [ -z "$( dmidecode -t processor | grep HTT)" ]; then
			write_log "HyperThreading/AMT isn't available on this machine"
			ret="0"
		else
			write_log "This server is HyperThreading capable, however, it is disabled"
			ret="0"
		fi
	else
